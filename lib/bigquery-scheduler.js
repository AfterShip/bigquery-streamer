...
Scheduler.prototype.scan = function () {
	...
	var init_promise;
	//cluster mode
	if(promisified_redis_client.nodes)
	{
		var tasks =  _.map(Object.keys(promisified_redis_client.nodes), function(node_name){
			return promisified_redis_client.nodes[node_name].keys(self.params.config.redis_namespace + ':*')
		});

		init_promise = Promise.all(tasks).then(function(keys_arr){
			var all = [];
			for (var i = 0; i < keys_arr.length; i++) {
				all = all.concat(keys_arr[i])
			}
			return all;
		});
	}else
	{
		init_promise = promisified_redis_client.keysAsync(this.params.config.redis_namespace + ':*')
	}
		init_promise
		.then(function (keys) {
			...
			return Promise.delay(self.params.config.schedule_interval);
		}).catch(function (e) {
			console.error(e);
		})
		.then(function () {
