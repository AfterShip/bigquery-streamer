...
class Sender {
	...
	start() {
		...
		return co(function*() {
			while (!self.should_stop) {
				let job = yield self.job_pool.pullAsync();
				try {
	...
	* doJob(job) {
		let key = job.key;
		let res = yield this.batchPop(this.services.redis_client, key, this.config.streamer.send_batch_size);
		if (!res.data || !res.data.length) {
			return null;
		}

		let bq_params = this.getBQParams(key);
		let data = yield this.preProcessData(res.data, this.config.streamer.max_row_size, bq_params.table_definition);
		if (data.length === 0) {
			return null;
		}

		let bq_res = yield this.sendToBigQuery({
		...
		if (bq_res[1] && bq_res[1].insertErrors) {
			this.logger.error(new Date(), `Data: ${JSON.stringify(data)} Insert Error: ${key} ${JSON.stringify(bq_res[1].insertErrors)}`);
		}

		return {
			sent: data.length,
			left: yield this.services.redis_client.llen(key),
			dataset: bq_params.dataset_id
		};
	}

	createTableId(table_name_format, key) {
	...
	getBQParams(full_key) {
		...
		const subject = key_parts[1];
		const version = key_parts[2];
		const key = key_parts[3];

		if (subject && key && version && this.config.streamer.table_definitions[subject][version]) {
			var table_id = this.createTableId(this.config.streamer.table_name_format, key);
			var dataset_id = `${this.config.streamer.dataset_namespace}_${subject}_${version}`;
			var table_definition = this.config.streamer.table_definitions[subject][version];
			return {
	...
	* batchPop(redis_client, key, size) {
		const res = yield redis_client.multi()
			.lrange(key, 0, size - 1)
			.ltrim(key, size, -1)
			.exec();
		...
		return retry(function*() {
			...
			try {
				...
				if (e.message.includes('Not found: Table')) {
					let expirationTime = params.table_definition.ttl ? moment().add(params.table_definition.ttl, 'days').valueOf() : null;
					try {
					...
					// if not table or dataset not found but 404 error, throw it
					self.logger.error(new Date(), e);
					throw e;
				}
			}
