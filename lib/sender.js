		...
		return co(function*() {
			...
			if (res.data && res.data.length) {
				...
				if (bq_res[1] && bq_res[1].insertErrors) {
					throw new Error(`Data: ${JSON.stringify(data)} Insert Error:  ${key} ${JSON.stringify(bq_res[1].insertErrors)}`);
				}
				return {
					sent: data.length,
			...
			try {
				...
				if (e.code === 404) {
					if (e.message.includes("Not found: Table")) {
						...
						var expirationTime = params.table_definition.ttl ? moment().add(params.table_definition.ttl, 'days').valueOf() : null;
						yield params.bq_client.createDatasetAsync({id: params.dataset_id, location: (params.dataset_location || "US")});
						yield dataset.createTableAsync({
							id: params.table_id,
							schema: {
								fields: params.table_definition.fields
