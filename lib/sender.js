...
class Sender {
	...
	preProcessData(records, max_row_size, table_definition) {
		...
		for (let raw_record of records) {
			...
			_.forOwn(record, function (value, key) {
				...
				if(key !== "cargo"){
					if ((!field_map.hasOwnProperty(key) && (value !== null || value !== undefined))
						...
						|| (table_definition.field_map[key] === "TIMESTAMP" && typeof value !== 'string')
	...
	sendToBigQuery(params) {
		...
		}, {retries: params.retries || 10, interval: params.retry_interval || 500, factor: 1.6});
	}
