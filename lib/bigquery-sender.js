...
Sender.prototype.send = function() {
	...
	var resource_paths;
	pop_multiple(promisified_redis_client, that.key, params.config.send_batch_size)
		.then(function(items) {
			...
			if (items && items.length && items.length > 0) {
				...
				resource_paths = getResourcePaths(that.key,that.params.config);
				...
				if (archive_items.length > 0) {
					...
					gcs.objects.insert(
						...
						function (err, result) {
							...
							{
								console.log('[gcs]',that.key,' sent ', new Date())
							}
						}
					)
		...
		.then(function(items) {
			...
			if (items && items.length && items.length > 0) {
				return send_to_bigquery(params, items, resource_paths)
					.then(function() {
						return items.length;
					})
					.catch(function(e) {
		...
		.then(function(length) {
			if (length >= params.config.send_batch_size) {
				var delay = params.config.min_flush_interval;
			} else {
				var delay = params.config.min_flush_interval + (params.config.max_flush_interval - params.config.min_flush_interval) * (1 - length / params.config.send_batch_size);
			}
			if (isNaN(delay)) {
				delay = Math.round(params.config.max_flush_interval) || 1000;
			}

			delay = Math.round(delay);
			console.log('[bq]','key:', that.key, ' sent_length:', length, ' delay:', delay, ' date: ', new Date());
 ...
 * @param params
 */
function send_to_bigquery(params, json_items, resource_paths) {
		...
		return send()
			...
			.spread(function(resp) {
				if (resp && resp.insertErrors) {
					throw new Error(that.key + ' insert failed: ' + JSON.stringify(resp || {}));
				} else {
					return true;
				}
			})
